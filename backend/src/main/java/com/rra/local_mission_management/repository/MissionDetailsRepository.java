...
public interface MissionDetailsRepository extends JpaRepository<MissionDetails, Long> {
    ...
    @Query("SELECT m FROM MissionDetails m WHERE m.employee = :employee AND "
            + "((m.startDate <= :endDate AND m.endDate >= :startDate)) AND m.status NOT IN ('MISSION_ORDER_REJECTED')")
    ...
    @Query("SELECT m FROM MissionDetails m " +
    "WHERE NOT EXISTS(SELECT 1 FROM MissionDetails msn WHERE msn.employee.employeeId=:employeeId and msn.status IN  ('MISSION_PAYMENT_BATCH_CREATED') AND msn.reportStatus IN ('NOT_SUBMITTED')) AND (m.employee.employeeId = :employeeId " +
    "AND m.endDate <= :deadline " +
    "AND m.reportStatus = 'NOT_SUBMITTED' " +
    "AND m.status NOT IN ('SUBMITTED_FOR_APPROVAL', 'MISSION_ORDER_RETURNED', 'MISSION_ORDER_REJECTED', 'MISSION_ORDER_CLEARANCE_RECORD_CANCELLED'))")
    List<MissionDetails> findPendingMissions(@Param("employeeId") String employeeId, @Param("deadline") LocalDate deadline);
}
